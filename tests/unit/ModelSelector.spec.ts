...
describe("ModelSelector", () => {
  ...
  it("shows dropdown items when open", async () => {
    ...
    expect(dropdownItems[0].find(".model-description").text()).toBe(
      "GPT-4 and GPT-3.5"
    );

    expect(dropdownItems[1].find(".model-name").text()).toBe(
      "Anthropic Claude"
    );
    expect(dropdownItems[1].find(".model-description").text()).toBe(
      "Claude 3 family"
    );

    expect(dropdownItems[2].find(".model-name").text()).toBe("Google Gemini");
    expect(dropdownItems[2].find(".model-description").text()).toBe(
      "Gemini Pro"
    );
  });
